...
def run( translator, data_loader, mode, optimizer, device, epoch, logfile, starttime, decode ):
    ...
    for i, (query, t, s) in enumerate(data_loader):
        ...
        if mode == 'train':
            wandb.log({f"Average_{mode}_MRR": hits[10]/hits[12]})
...
def main():
    ...
    kg_path = "/data/cyl/MyPaper/No_sub_Ruleformer4UKG/DATASET/NL27K"
    ...
    train_loader = torch.utils.data.DataLoader( dataset=train_data, batch_size=batch_size, shuffle=True, collate_fn=MyDataset.collate_fn, )  # , num_workers=16
    ...
    decode_rule = False
    translator = Translator(model=model, body_len=3, device=device, kg=kg).to(device)
    ...
    if not os.path.exists(f'EXPS/{logfile}'):
        os.mkdir(f'EXPS/{logfile}')
    ...
    # 指定要写入的文件路径
    file_path = f'EXPS/{logfile}/config.txt'
